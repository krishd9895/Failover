...
# 3. Define your target string/parameters inside the 'TARGET' parameter.
###############################################################################
"""

# --- LAUNCH MODE CONFIGURATION ---
LAUNCH_MODE = "PYTHON"       # Options: "PYTHON", "DOCKER_COMPOSE", or "RAW"
TARGET = ""                  # PYTHON: "main.py"/"bot.py" | DOCKER_COMPOSE: "-f prod.yml" (Optional)
...
# Relocate .machine_id out of the project folder to the host system directory
# This natively isolates the unique identity token if the project folder is cloned/copied
...
if mode_upper == "PYTHON":
    ...
    # CORRECT ARGS PLACEMENT: docker compose [flags] up
    ...
    print(f"CRITICAL CONFIGURATION ERROR: Invalid LAUNCH_MODE '{LAUNCH_MODE}' specified!", file=sys.stderr)
    sys.exit(1)
...
class NodeLoggerAdapter(logging.LoggerAdapter):
    def process(self, msg, kwargs):
        kwargs["extra"] = {"node_id": f"{socket.gethostname()}:{PERSISTENT_MACHINE_UUID[:8]}"}
        return msg, kwargs
...
def terminate_child():
    """Wipes out container spaces via explicit down calls before processing signal fallbacks."""
    global child_process
    
    # 1. Deterministic Docker Stack Teardown (Only triggers if explicitly in DOCKER_COMPOSE mode)
    if LAUNCH_MODE.strip().upper() == "DOCKER_COMPOSE":
        ...
        try:
            ...
            logger.info(f"Executing stack teardown vector: {down_args}")
    ...
    # 2. Cross-platform process tree cleanup
    ...
    try:
        ...
        if update_telemetry:
            update_modifier["$set"].update({
                ...
                "resolved_command": START_COMMAND_STRING,
...
def main():
    ...
    print(f"======================================================================", flush=True)
    print(f"🔥 HA PROCESS WATCHDOG ACTIVE | Launch Mode: {LAUNCH_MODE}", flush=True)
    print(f"Service ID : {SERVICE_ID}", flush=True)
    print(f"Node ID    : {socket.gethostname()}:{PERSISTENT_MACHINE_UUID[:8]} ({IDE_CONTEXT})", flush=True)
    print(f"Resolved Vector Array: {final_exec_args}", flush=True)
    print(f"======================================================================\n", flush=True)
